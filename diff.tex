...
%DIF DEL /dev/fd/63      Mon Sep 29 12:48:16 2014
%DIF ADD main_text.tex   Mon Sep 29 12:48:07 2014
...
Ordinary differential equation (ODE) models of gene regulation are capable of describing the amplitude and phase-resetting behavior of single cells, but fail to capture the collective dynamics of a population \DIFaddbegin \DIFadd{of }\DIFaddend oscillators.
...
%DIFDELCMD <   \frac{dx}{dt} = f(x(t), p)
%DIFDELCMD <   \label{eq:odefn}
...
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \label{eq:odefn}
\end{equation}
\DIFaddend in which $x(t)$ represents the concentrations of the state variables, such as mRNA and protein concentrations, $f$ contains information on the production, degradation, and reactivity of the states, and $p$ are the kinetic parameters which govern reaction kinetics.
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \lim_{t \to \infty} \left[ x(t + T) - x(t) \right] = 0
%DIFDELCMD <   \label{eq:limit}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \label{eq:limit}
\end{equation}
\DIFaddend The period is the smallest $T > 0$ for which Eq.~\ref{eq:limit} holds.
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \tilde{t} = \frac{2\pi}{T}t; \quad \tilde{f} = \frac{T}{2\pi}\;f; \quad \frac{dx}{d\tilde{t}} = \tilde{f}(x(\tilde{t}), p)
%DIFDELCMD <   \label{eq:that}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \label{eq:that}
\end{equation}
\DIFaddend The phase variable $\theta$ is therefore defined on the limit cycle as $\theta = \tilde{t}\mod 2\pi$, with $\theta = 0$ assigned to a unique and identifiable point.
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   x(0) \coloneqq x^\gamma(\theta_0) + \Delta x(0)
%DIFDELCMD <   \label{eq:stateperturbation}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
\end{equation}
\DIFaddend This trajectory evolves according to Eq.~\ref{eq:odefn}, eventually returning to $x^\gamma$.
It is useful to express this trajectory by the deviation from the limit cycle:
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \Delta x(\tilde{t}) = x(\tilde{t}) - x^\gamma(\tilde{t} + \theta_0)
%DIFDELCMD <   \label{eq:delxt}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
%DIFDELCMD <   x_{\tilde{d}}(-\tilde{d}) &= x^\gamma(\theta_0 - \tilde{d}) \\
%DIFDELCMD <   \frac{dx_{\tilde{d}}}{d\tilde{t}} &= \tilde{f}(x_{\tilde{d}}(\tilde{t}), p + \Delta p) \label{eq:ode_pert}
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
...
\end{align}
\DIFaddend The pulse trajectory $x_{\tilde{d}}(\tilde{t})$ is then integrated from $\tilde{t} = -\tilde{d} \to 0$, at which point the pulse is removed.
A temporary parameter pulse is therefore equivalent to a state perturbation, but with the perturbation at $t = 0$ defined by:
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \Delta x(0) = x_{\tilde{d}}(0) - x^\gamma(\theta_0)
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
The sensitivity matrix\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{equation}
%DIFDELCMD <   S(t) = \frac{dx(t)}{dx(0)} = \lim_{\Delta x(0) \to 0}\frac{\Delta x(t)}{\Delta x(0)}
%DIFDELCMD <   \label{eq:senslimit}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{,
  ...
  \label{eq:senslimit}
\end{equation}
\DIFaddend can be calculated directly by integrating
\DIFdelbegin \DIFdel{:
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \frac{d}{d\tilde{t}}\frac{d\theta}{dp} = \frac{d\theta}{dx}\frac{d\tilde{f}}{dp} 
%DIFDELCMD <   \label{eq:pPRCequiv}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \label{eq:pPRCequiv}
\end{equation}
\DIFaddend This result follows from the fact that in the limit of an infinitely short and small parameter pulse, $\Delta x(0) \to \nicefrac{d\tilde{f}}{dp}\,\tilde{d}\, \Delta p$.
...
As with all probability density functions\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{equation}
%DIFDELCMD <   \int_0^{2\pi} p(\theta, \tilde{t}) \; d\theta = 1
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{,
}\begin{equation}
...
\end{equation}
\DIFaddend The shape of $p(\theta, \tilde{t})$ changes as the cells advance in time.
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \frac{\partial p}{\partial \tilde{t}} = \frac{\partial p}{\partial \theta} + d\frac{\partial^2 p}{\partial \theta^2}\\
%DIFDELCMD <   \label{eq:pde}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \label{eq:pde}
\end{equation}
\DIFaddend Due to the rescaling of $t$, the mean period of the population is $2\pi$.
...
\DIFdelbegin %DIFDELCMD < \begin{align}
%DIFDELCMD <   \text{BCs:}\quad p(0, \tilde{t}) &= p(2\pi, \tilde{t}) \\
%DIFDELCMD <   \frac{\partial p}{\partial \theta}(0, \tilde{t}) &= \frac{\partial p}{\partial \theta}(2\pi, \tilde{t}) \\
%DIFDELCMD <   \text{IC:}\quad p(\theta, 0) &= \phi(\theta)\label{eq:pde_ic}
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}
  ...
  \label{eq:pde_ic}
\end{align}
\DIFaddend The solution of Eqs.~\ref{eq:pde}-\ref{eq:pde_ic} is well-characterized, with $p(\theta, \tilde{t})$ evolving in time as the convolution of the initial conditions with a wrapped normal distribution with mean $\tilde{t}$ and standard deviation $\sqrt{2d\tilde{t}}$ \cite{Chirikjian2009}:
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   p(\theta, \tilde{t}) = \phi(\theta) * \mathcal{WN}(\theta; \tilde{t},
%DIFDELCMD <   \sqrt{2\, d\, \tilde{t}})
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \sqrt{2\, d\, \tilde{t}}),
\end{equation}
\DIFaddend in which the wrapped normal distribution \cite{Mardia2009} is defined as
\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{equation}
%DIFDELCMD <   {\cal WN}(\theta; \mu, \sigma) =
%DIFDELCMD <   \frac{1}{\sigma\sqrt{2\pi}}\sum_{k=-\infty}^\infty \exp\left[\frac{-(\theta
%DIFDELCMD <   - \mu + 2\pi k)^2}{2\sigma^2}\right]
%DIFDELCMD <   \label{eq:wrapped_normal}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
\end{equation}
\DIFaddend Since the convolution of two normal distributions is also a normal distribution, it is efficient when possible to describe $\phi(\theta)$ as a normal distribution with mean $\mu_0$ and standard deviation $\sigma_0$, such that $p(\theta, \tilde{t})$ can be found analytically through
\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{equation}
%DIFDELCMD <   p(\theta, \tilde{t}) = \mathcal{WN}(\theta; \mu_0 + \tilde{t},
%DIFDELCMD <   \sqrt{\sigma_0^2 + 2d^2\tilde{t}^2})
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
To define an amplitude change metric for such a case, we compare a perturbed trajectory $x(t)$ to a phase-shifted limit cycle reference $y(t)$, for which $x(t) \to y(t)$ for sufficiently long times\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{equation}
%DIFDELCMD <   \begin{aligned}
%DIFDELCMD <     \Delta A (x(t), y(t)) &\coloneqq \int_0^\infty (x(t) - \mu)^2 - (y(t) - \mu)^2 \; dt\\
%DIFDELCMD <     &= \int_0^\infty h(t) \; dt
%DIFDELCMD <   \end{aligned}
%DIFDELCMD <   \label{eq:ampchangedefinition}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdel{This amplitude metric was chosen over alternatives, such as peak-trough distance, for its analytical tractability}\DIFdelend .
Since $x(t)$ approaches the reference as $t \to \infty$, the means of both trajectories are equal \DIFdelbegin \DIFdel{.
The mean of the limit cycle can therefore be calculated as:
}%DIFDELCMD < \begin{equation}
%DIFDELCMD <   \mu \coloneqq \int_0^{2\pi} \frac{x^\gamma(\theta)}{2\pi} \; d\theta
%DIFDELCMD <   \label{eq:mu}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{and can be calculated by
}\begin{equation}
  \mu \coloneqq \int_0^{2\pi} \frac{x^\gamma(\theta)}{2\pi} \; d\theta.
  \label{eq:mu}
\end{equation}
\DIFadd{The amplitude change metric is defined as
}\begin{equation}
  \begin{aligned}
...
\end{equation}
\DIFadd{This amplitude metric was chosen over alternatives, such as peak-trough distance, for its analytical tractability.
}\DIFaddend The integrand in Eq.~\ref{eq:ampchangedefinition}, abbreviated by $h(t)$, compares the variances of the reference and perturbed trajectories.
...
%DIFDELCMD <   \frac{dA}{dx} \coloneqq \lim_{\Delta x(0) \to 0} \frac{\Delta A\left(x(\tilde{t}),\ x^\gamma(\tilde{t} + \theta_0 + \Delta\theta)\right)}{\Delta x(0)} = \int_0^\infty \lim_{\Delta x(0) \to 0} \frac{h(t)}{\Delta x(0)} \; dt
%DIFDELCMD <   \label{eq:sARC}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
\DIFdelbegin %DIFDELCMD < \begin{align}
%DIFDELCMD <   x^\gamma(t_\theta + \Delta\theta) &= x^\gamma(t_\theta) +
%DIFDELCMD <   \frac{dx^\gamma(t_\theta)}{d\theta}\Delta\theta + O(\Delta\theta^2)\\
%DIFDELCMD <   &= x^\gamma(t_\theta) + \tilde{f}\left(x^\gamma(t_\theta)\right)\Delta\theta +
%DIFDELCMD <   O(\Delta\theta^2)
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}
  ...
  O(\Delta\theta^2).
\end{align}
\DIFaddend Simplifying the integrand in Eq.~\ref{eq:sARC}\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{align}
%DIFDELCMD <   \frac{h(t)}{\Delta x(0)} &= \frac{1}{\Delta x(0)} \left[\left(x^\gamma(t_\theta) +
%DIFDELCMD <     \Delta x(\tilde{t}) - \mu\right)^2 - \left(x^\gamma(t_\theta +
%DIFDELCMD <     \Delta \theta) - \mu\right)^2 \right]\\
%DIFDELCMD <   &= \frac{1}{\Delta x(0)} \left[\left(x^\gamma(t_\theta) +
%DIFDELCMD <     \Delta x(\tilde{t}) - \mu\right)^2 - \left(x^\gamma(t_\theta) + \tilde{f}\left(x^\gamma(t_\theta)\right)\Delta\theta - \mu\right)^2 \right]\\
%DIFDELCMD <   &= \frac{1}{\Delta x(0)} \left[\left(\Delta x(\tilde{t}) -
%DIFDELCMD <     \tilde{f}\left(x^\gamma(t_\theta)\right)\Delta\theta\right) \left(\Delta
%DIFDELCMD <     x(\tilde{t}) + \tilde{f}\left(x^\gamma(t_\theta)\right)\Delta\theta +
%DIFDELCMD <     2(x^\gamma(t_\theta) - \mu)\right) \right]
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{,
}\begin{align}
...
\end{align}
\DIFaddend Taking the limit of this integrand as $\Delta x(0) \to 0$ cancels several differential terms, and allows the remainder to be substituted with quantities that can be calculated using ODE sensitivity analysis:
\DIFdelbegin %DIFDELCMD < \begin{align}
%DIFDELCMD <   \lim_{\Delta x(0) \to 0} \frac{h(t)}{\Delta x(0)} &= 2\left(\lim_{\Delta x(0) \to 0}\frac{\Delta x(\tilde{t})}{\Delta x(0)} - \tilde{f}\left(x^\gamma(t_\theta)\right)\lim_{\Delta x(0) \to 0}\frac{\Delta\theta}{\Delta x(0)}\right) \left(x^\gamma(t_\theta) - \mu\right)\\
%DIFDELCMD <   &= 2\left(S(\tilde{t}) - \tilde{f}(x^\gamma(t_\theta))\frac{d\theta}{dx}\right)\left(x^\gamma(t_\theta) - \mu\right)
%DIFDELCMD <   \label{eq:sens_sub}
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \frac{dA}{dx} = \int_0^\infty 2\left(S(\tilde{t}) - \tilde{f}(x^\gamma(t_\theta))\frac{d\theta}{dx}\right)\left(x^\gamma(t_\theta) - \mu\right) \; dt
%DIFDELCMD <   \label{eq:siARC}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \label{eq:siARC}
\end{equation}
\DIFaddend Here, the first term of the integrand $(S - \tilde{f}\dot{\theta})$ tracks the distance from the perturbed trajectory to the limit cycle, which decays to zero as $t \to \infty$.
...
%DIFDELCMD <   \frac{d}{d\tilde{t}}\frac{dA}{dp} \coloneqq \lim_{\tilde{d},\; \Delta p \to 0} \frac{\Delta A}{\tilde{d}\, \Delta p}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  \frac{d}{d\tilde{t}}\frac{dA}{dp} \coloneqq \lim_{\tilde{d},\; \Delta p \to 0} \frac{\Delta A}{\tilde{d}\, \Delta p},
\end{equation}
\DIFaddend and may be calculated from the state-impulse version with the following relationship:
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \frac{d}{d\tilde{t}}\frac{dA}{dp} = \frac{dA}{dx}\frac{d\tilde{f}}{dp}
%DIFDELCMD <     \label{eq:piARC}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  \frac{d}{d\tilde{t}}\frac{dA}{dp} = \frac{dA}{dx}\frac{d\tilde{f}}{dp}.
    \label{eq:piARC}
\end{equation}
\DIFaddend As with Eq.~\ref{eq:pPRCequiv}, this equivalency reflects the fact that for a pulse of infinitely short duration, a parameter change is equivalent to changing the state of the system along the direction specified by the Jacobian.
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \hat{p}(\theta, \tilde{t})\; dg(\theta) = p(\theta, \tilde{t})\; d\theta
%DIFDELCMD <   \label{eq:pdfinversion}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
\end{equation}
\DIFaddend However, it is easier to estimate the mean and standard deviation of the perturbed population $\hat{p}(\theta, \tilde{t})$ using directional statistics \cite{Mardia2009}.
...
%DIFDELCMD <   z &\coloneqq \int_0^{2\pi} e^{i\theta} p(\theta, \tilde{t}) \; d\theta \label{eq:zbar}\\
%DIFDELCMD <   \hat{z} &\coloneqq  \int_0^{2\pi} e^{i\theta} \hat{p}(\theta, \tilde{t}) \; d\theta =
%DIFDELCMD <   \int_0^{2\pi} e^{i g(\theta)} p(\theta, \tilde{t}) \; d\theta
%DIFDELCMD <   \label{eq:zhat}
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}
...
%DIFDELCMD <   \Delta \bar{\theta} &= \angle z - \angle \hat{z} \\
%DIFDELCMD <   \Delta \rho &= |z| - |\hat{z}|
%DIFDELCMD <   \label{eq:popampchange}
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}
...
\end{align}
\DIFaddend Population-level PRCs and ARCs can be tabulated by solving Eqs.~\ref{eq:zbar}-\ref{eq:popampchange} for populations $p(\theta, \tilde{t})$ with different mean phases.
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD <   \bar{x}(\tilde{t}) = \int_0^{2\pi} x^\gamma(\theta) p(\theta, \tilde{t}) \;
%DIFDELCMD <   d\theta
%DIFDELCMD <   \label{eq:xbar}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
\DIFdelbegin %DIFDELCMD < \begin{align}
%DIFDELCMD <   \bar{x}(\tilde{t}) &= \frac{1}{\sqrt{4\pi d\tilde{t}}}\int_{-\infty}^{\infty}
%DIFDELCMD <   \cos(x) \exp\left(-\frac{(x - \tilde{t})^2}{4d\tilde{t}}\right)\; dx \\
%DIFDELCMD <   &= \Re\left[\frac{1}{\sqrt{4\pi d\tilde{t}}}\int_{-\infty}^{\infty}
%DIFDELCMD <     \exp(ix) \exp\left(-\frac{(x - \tilde{t})^2}{4d\tilde{t}}\right)\; dx\right] \\
%DIFDELCMD <     &= \Re\left[e^{(i - d)\tilde{t}}\right]\\
%DIFDELCMD <     &= e^{-d\tilde{t}} \cos(\tilde{t}) \label{eq:expsin}
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}
  ...
  &= \Re\left[\frac{1}{\sqrt{4\pi d\tilde{t}}}\int_{-\infty}^{\infty} \exp(ix) \exp\left(-\frac{(x - \tilde{t})^2}{4d\tilde{t}}\right)\; dx\right] \\
    ...
    \label{eq:expsin}
\end{align}
\DIFaddend Due to the smoothing effect of phase diffusion, higher frequency sinusoidal components of the limit cycle are damped faster than lower frequency components, resulting in an exponentially decaying sinusoids even for limit cycles that are not sinusoidal in sufficiently disperse populations.
...
%DIFDELCMD <   \hat{x}_{ss}(\tilde{t}) = \int_0^{2\pi} x^\gamma(\theta) \hat{p}(\theta, \tilde{t}) \; d\theta
%DIFDELCMD <   \label{eq:xhatss}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
...
\DIFdelbegin %DIFDELCMD < \begin{align}
%DIFDELCMD <   \delta x(\theta_0, \tilde{t}) &\coloneqq x(\tilde{t}) - x^\gamma(\tilde{t} + \theta_0 + \Delta \theta) \\
%DIFDELCMD <   \therefore\; \lim_{\tilde{t} \to \infty} \delta x(\theta, \tilde{t}) &= 0
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}
...
\end{align}
\DIFaddend Since the perturbed trajectory ultimately converges with the phase-adjusted reference, deviations will converge to zero.
...
%DIFDELCMD <   \hat{x}(\tilde{t}) = \int_0^{2\pi} x^\gamma(\theta)\hat{p}(\theta, \tilde{t}) + \delta x(\theta, \tilde{t})p(\theta, \tilde{t}) \; d\theta
%DIFDELCMD <   \label{eq:xhat}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}
  ...
  \label{eq:xhat}
\end{equation}
\DIFaddend Here the first term is equivalent to $\hat{x}_{ss}(\tilde{t})$, the steady-state perturbed trajectory, while the second term decays to zero as individual oscillators return to their steady-state amplitude.
...
We selected the degradation rate of {\itshape Per} mRNA from the feasible parameters \DIFdelbegin \DIFdel{, }\DIFdelend due to PER's known induction by CREB following photo-perturbation \cite{Tischkau2003}.
\DIFaddbegin \DIFadd{While not an exact match of the experimental system, reducing degradation rate yields a similar response to increasing transcription.
}\DIFaddend The parameter was reduced by $28.5\%$ during the light pulses, a value fit to maximize light-induced desynchrony.
...
As higher-frequency features, these bimodal distributions dissipate as the phases diffuse\DIFaddbegin \DIFadd{.
Amplitude change in this case is mediated both at the population and single-cell level, as indicated by their representative ARCs. The contributions from each source are summarized in Fig.~S4}\DIFaddend .
