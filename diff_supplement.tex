...
%DIF DEL /dev/fd/63              Tue Sep 30 10:07:02 2014
%DIF ADD supplemental_info.tex   Sat Sep 27 14:09:55 2014
...
($P=2$).
\DIFaddbegin \DIFadd{It should be noted that the use of the stochastic simulation algorithm for reactions with non-elementary propensities may inaccurately represent the noise of the full system.
In this case, we simply fit the noise characteristics of the approximated model (using the volume parameter) to yield physiologically realistic desynchronization rates. The volume of the stochastic simulations was chosen as \mbox{%DIFAUXCMD
$\Omega = 250$
}%DIFAUXCMD
.
}\DIFaddend }
...
\DIFaddFL{(}{\bfseries \DIFaddFL{A}}\DIFaddFL{) The simulated control trajectory as shown in Fig.~5 is plotted again as \mbox{%DIFAUXCMD
$\bar{x}(\tilde{t})$
}%DIFAUXCMD
.
The simulated light-sensitive trajector is also replotted as \mbox{%DIFAUXCMD
$\hat{x}(\tilde{t})$
}%DIFAUXCMD
, which incorperates both single-cell and population-level effects.
The difference between this trajectory and the steady-state perturbed trajectory following both pulses, \mbox{%DIFAUXCMD
$\hat{x}_{ss}(\tilde{t})$
}%DIFAUXCMD
